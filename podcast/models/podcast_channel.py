...
class PodcastChannel(models.Model):
    ...
    duration = fields.Integer(string='Total Duration (minutes)', compute='_compute_total_duration', store=True)
    formatted_duration = fields.Char(string='Duration', compute='_compute_formatted_duration', store=True,
                                     readonly=False)
    ...
    episodes = fields.One2many('podcast.episode', 'channel_id', string='Episodes')
    total_episodes = fields.Integer(compute='_compute_episodes_length', store=False)
    links_length = fields.Integer(string='Links Length', compute='_compute_links_length', store=False, copy=False)
    ...
    def _compute_links_length(self):
        links_length = self.env['podcast.link'].search_count([])
        for record in self:
            record.links_length = links_length

    def action_view_episodes(self):
        action = self.env['ir.actions.actions']._for_xml_id('podcast.action_website_podcast_episode')
        action['context'] = {
