...
from ..tools.shenoto_api import ShenotoApi


class PodcastEpisodeWizard(models.TransientModel):
    _name = "website.podcast.episode.wizard"
    _description = "Create Episode from Shenoto API"

    full_url = fields.Char(
        string="Full Shenoto URL", required=True, readonly=False, trim=True
    )
    ...
    def action_confirm(self):
        ...
        if not self.full_url:
            raise UserError(
                _("You cannot continue without filling the required field!")
            )
        ...
        self.env["website.podcast.episode"].create(
            {
